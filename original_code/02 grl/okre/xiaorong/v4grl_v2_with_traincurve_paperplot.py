...
@torch.no_grad()
def eval_bce_loss(model_enc: nn.Module, model_clf: nn.Module, loader: DataLoader, device: str) -> float:
    """Binary CE loss (with logits) on a labeled loader."""
    model_enc.eval(); model_clf.eval()
    losses: List[float] = []
...
def _smooth_ma(arr, window: int = 10):
...
def plot_stage1_adversarial_curves(log_csv: str,
                                   out_png: str,
                                   stable_loss: float = 0.693,
                                   ma_window: int = 10):
    """
    Paper-friendly DANN/GRL training dynamics plot.

    Panel (a): task-side losses (smoothed) + (optional) val task loss.
    Panel (b): domain loss (smoothed, with 0.693) + domain accuracy (smoothed, with 0.5),
               and lambda schedule overlay (twin axis).

    Notes:
      - Uses moving-average smoothing (default window=10) to reduce adversarial oscillation noise.
      - Uses domain accuracy instead of mean probability (mean prob can be biased by batch/domain mix).
    """
    df = pd.read_csv(log_csv)

    it = df["iter"].to_numpy()
    dom_loss = df["dom_loss"].to_numpy()
    task_loss = df["task_loss"].to_numpy()
    total_loss = df["total_loss"].to_numpy()
    val_task_loss = df.get("val_task_loss", pd.Series([float("nan")] * len(df))).to_numpy()
    dom_acc = df.get("dom_acc", pd.Series([float("nan")] * len(df))).to_numpy()
    lambd = df.get("lambda", pd.Series([float("nan")] * len(df))).to_numpy()

    # Smooth
    dom_loss_s = _smooth_ma(dom_loss, ma_window)
    task_loss_s = _smooth_ma(task_loss, ma_window)
    total_loss_s = _smooth_ma(total_loss, ma_window)
    dom_acc_s = _smooth_ma(dom_acc, ma_window) if np.isfinite(dom_acc).any() else dom_acc
    lambd_s = _smooth_ma(lambd, ma_window) if np.isfinite(lambd).any() else lambd

    # val task loss is sparse (NaNs). Smooth it but keep NaNs where there is no value.
    val_s = _smooth_ma(val_task_loss, ma_window)
    val_mask = np.isfinite(val_task_loss)

    fig, axes = plt.subplots(2, 1, figsize=(7.2, 6.0), sharex=True)

    # ---------------- (a) Task-side losses ----------------
    ax = axes[0]
    ax.plot(it, task_loss_s, label="Task loss (train, source)")
    ax.plot(it, total_loss_s, label="Total loss (train)")
    if val_mask.any():
        ax.plot(it[val_mask], val_s[val_mask], label="Task loss (val)")

    ax.set_ylabel("Loss")
    ax.grid(True, linewidth=0.3, alpha=0.35)
    ax.legend(loc="upper right", frameon=False)

    # ---------------- (b) Domain-side dynamics ----------------
    ax = axes[1]
    # Domain loss on left y-axis
    ax.plot(it, dom_loss_s, label="Domain loss", linewidth=1.8)
    ax.axhline(stable_loss, linestyle="--", linewidth=1.0)
    ax.text(it.max() * 0.70, stable_loss + 0.015, f"Theoretical loss {stable_loss:.3f}")

    ax.set_ylabel("Loss")
    ax.grid(True, linewidth=0.3, alpha=0.35)

    # Domain accuracy on right y-axis
    ax2 = ax.twinx()
    if np.isfinite(dom_acc).any():
        ax2.plot(it, dom_acc_s, linestyle="-", label="Domain acc", linewidth=1.8)
        ax2.axhline(0.5, linestyle="--", linewidth=1.0)
        ax2.text(it.max() * 0.70, 0.5 + 0.02, "Chance acc 0.5")
        ax2.set_ylabel("Accuracy")

    # Lambda schedule (overlay on right axis, faint)
    if np.isfinite(lambd).any():
        ax2.plot(it, lambd_s, linestyle=":", linewidth=1.6, label="Lambda (schedule)")
        # Put a small label near the end (avoid clutter)
        ax2.text(it.max() * 0.70, float(np.nanmax(lambd_s)) + 0.02, "Lambda schedule")

    axes[1].set_xlabel("Iteration")

    # Unified legend for panel (b): gather handles from both axes
...
def train_stage1_oth_supervised_grl_v2(
    ...
    # ---- iter-level logging (for Fig.3-like curves) ----
    log_rows: List[Dict[str, float]] = []
    global_it = 0
    val_every = int(max(1, len(dl_tr) // 2))  # adjust if you want denser/sparser val points
    log_csv = out_dir / "stage1_grl_iterlog.csv"
    ...
    for ep in range(epochs):
        ...
        for xb_s, yb_s, _ in dl_tr:
            ...
            # ---- iter log ----
            with torch.no_grad():
                dom_prob_mean = float(torch.sigmoid(dlog).mean().item())

            vloss = float("nan")
            if (global_it % val_every) == 0:
                vloss = eval_bce_loss(enc, clf, dl_va, device)

            log_rows.append({
                "iter": int(global_it),
                "epoch": int(ep),
                "lambda": float(lambd),
                "task_loss": float(loss_task.item()),
                "dom_loss": float(loss_dom.item()),
                "total_loss": float(loss.item()),
                "dom_prob_mean": float(dom_prob_mean),
                "dom_acc": float(acc),
                "val_task_loss": float(vloss),
            })
            global_it += 1
    ...
    # dump iter log for plotting
    if len(log_rows) > 0:
        try:
            pd.DataFrame(log_rows).to_csv(log_csv, index=False)
            print(f"[Stage1-GRLv2] saved iter log: {log_csv}")
        except Exception as e:
            print(f"[Stage1-GRLv2] failed to save iter log: {e}")
...
def main(CFG: Dict):
    ...
    # Plot Stage1 training curves if the iter log exists
    try:
        log_csv = out_root / "stage1_grl_iterlog.csv"
        if log_csv.exists():
            out_png = out_root / "fig_stage1_grl_training_paper.png"
            plot_stage1_adversarial_curves(str(log_csv), str(out_png))
