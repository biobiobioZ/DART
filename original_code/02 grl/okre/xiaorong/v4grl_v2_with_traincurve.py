...
def plot_stage1_adversarial_curves(log_csv: str, out_png: str, stable_loss: float = 0.693):
    ...
    # (a) loss curves
    ax = axes[0]
    ax.plot(it, dom_loss, label="域分类器损失")
    ax.plot(it, total_loss, label="预测器(训练总损失)")
    ax.plot(it, task_loss, label="预测器(训练损失-源域)")

    m = ~np.isnan(val_task_loss)
    if np.any(m):
        ax.plot(it[m], val_task_loss[m], label="预测器(验证总损失)")

    ax.axhline(stable_loss, linestyle="--", linewidth=1.0)
    ax.text(it.max() * 0.72, stable_loss + 0.01, f"理论稳定值{stable_loss:.3f}")
    ...
    ax = axes[1]
    ax.scatter(it, dom_loss, s=14, label="域分类器损失")
    ax.scatter(it, dom_prob, s=14, marker="s", label="域分类器概率")

    ax.axhline(stable_loss, linestyle="--", linewidth=1.0)
    ax.axhline(0.5, linestyle="--", linewidth=1.0)
    ax.text(it.max() * 0.62, stable_loss + 0.01, f"理论稳定值{stable_loss:.3f}")
    ax.text(it.max() * 0.62, 0.5 + 0.01, "理论预测概率0.5")

    ax.set_xlabel("迭代轮次")
    ax.set_ylabel("损失值, 概率")
    ax.grid(True, linewidth=0.3, alpha=0.4)
    ax.legend(loc="upper right", frameon=False)
