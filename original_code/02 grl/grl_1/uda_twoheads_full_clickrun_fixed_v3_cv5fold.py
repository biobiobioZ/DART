# uda_twoheads_full_clickrun_fixed_v3_cv5fold.py
# Click-Run: Global Stage0 (DAE, unlabeled-only, once) -> Global Stage1a/1b (OTH supervised, once)
#         -> Outer 5-fold CV on TNBC microarray labeled:
#              per fold: Stage2 (UDA + TWO adversarial heads + SSDA anchor) using inner-val for checkpoint,
#                        evaluate ONLY on outer-test (never used in training/selection)
#
# ✅ Two adversarial heads (DECOLLINEARIZED):
#   - platform head: ONLY within-TNBC (TNBC RNA-seq vs TNBC microarray) using z_bc + GRL
#   - cancer head  : ONLY within-RNAseq (OTH RNA-seq vs TNBC RNA-seq) using z_inv + GRL
#
# ✅ v3 critical:
#   (1) task head uses z_inv ONLY
#   (2) Stage2 uses TNBC_array_labeled (inner-train) as supervised anchor (lambda_tlab)
#   (3) warmup schedule for GRL / entropy / pseudo-label to prevent collapse
#   (4) stratified split for OTH train/val
#   (5) DAE pretrain on mixed unlabeled pool (OTH_tr + TNBC UL (+TNBC RNA UL)) [STRICT: no TNBC labeled]
#
# Outputs:
#   out_dir/
#     encoder_pretrained.pt
#     stage1b_best_shared.pt
#     genes.json
#     fold1..foldK/
#       checkpoint_uda_best_target.pt (best on inner-val)
#       checkpoint_uda_best_othval.pt (best on OTH-val, just for diagnosis)
#       metrics_outer_test.csv / roc_outer_test.png / pred_outer_test.csv
#       metrics_inner_val.csv / roc_inner_val.png / pred_inner_val.csv
#       metrics_oth_val.csv / roc_oth_val.png / pred_oth_val.csv
#       uda_auc_othval.png, uda_auc_tnbc_inner_val.png, etc.
#     cv_summary.csv

import json, math
from pathlib import Path
from typing import Optional, List

import numpy as np
import pandas as pd
import torch
import torch.nn as nn
import torch.nn.functional as F

from sklearn.metrics import roc_auc_score, roc_curve, accuracy_score, f1_score
from sklearn.model_selection import train_test_split, StratifiedKFold
from torch.utils.data import DataLoader, Dataset

import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt


# ----------------- Datasets -----------------
class LabeledDS(Dataset):
    """Return: x, y, id, platform_id, cancer_id"""
    def __init__(self, df: pd.DataFrame, gene_list: List[str], label_col: str, platform_id: int, cancer_id: int):
        X = df[gene_list].astype(np.float32).values
        y = df[label_col].astype(np.int64).values
        self.X = torch.from_numpy(X)
        self.y = torch.from_numpy(y)
        self.ids = df.index.astype(str).tolist()
        self.platform_id = int(platform_id)
        self.cancer_id = int(cancer_id)

    def __len__(self):
        return len(self.ids)

    def __getitem__(self, i):
        return self.X[i], self.y[i], self.ids[i], self.platform_id, self.cancer_id


class UnlabeledDS(Dataset):
    """Return: x, id, platform_id, cancer_id"""
    def __init__(self, df: pd.DataFrame, gene_list: List[str], platform_id: int, cancer_id: int):
        X = df[gene_list].astype(np.float32).values
        self.X = torch.from_numpy(X)
        self.ids = df.index.astype(str).tolist()
        self.platform_id = int(platform_id)
        self.cancer_id = int(cancer_id)

    def __len__(self):
        return len(self.ids)

    def __getitem__(self, i):
        return self.X[i], self.ids[i], self.platform_id, self.cancer_id


class TensorOnlyDS(Dataset):
    """Return: x only (for AE pretrain pool)"""
    def __init__(self, X: np.ndarray):
        self.X = torch.from_numpy(X.astype(np.float32))

    def __len__(self):
        return self.X.size(0)

    def __getitem__(self, i):
        return self.X[i]


# ----------------- Losses / Tools -----------------
class FocalLoss(nn.Module):
    def __init__(self, gamma=2.0, alpha=0.75):
        super().__init__()
        self.g = float(gamma)
        self.a = float(alpha)

    def forward(self, logits, y):
        ce = F.cross_entropy(logits, y, reduction="none")
        pt = torch.exp(-ce)
        return (self.a * (1 - pt) ** self.g * ce).mean()


class CenterLoss(nn.Module):
    def __init__(self, num_classes, feat_dim):
        super().__init__()
        self.centers = nn.Parameter(torch.randn(num_classes, feat_dim) * 0.1)

    def forward(self, z, y):
        return ((z - self.centers[y]) ** 2).sum(dim=1).mean()


class GradReverse(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, lambd):
        ctx.lambd = float(lambd)
        return x.view_as(x)

    @staticmethod
    def backward(ctx, grad):
        return -ctx.lambd * grad, None


def grl(x, lambd):
    return GradReverse.apply(x, lambd)


def orth_loss(z_inv, z_bc):
    """decorrelate z_inv and z_bc"""
    if z_inv.size(0) <= 1:
        return z_inv.new_tensor(0.0)
    z_inv_c = z_inv - z_inv.mean(dim=0, keepdim=True)
    z_bc_c  = z_bc  - z_bc.mean(dim=0, keepdim=True)
    C = torch.matmul(z_inv_c.t(), z_bc_c) / (z_inv_c.size(0) - 1)
    return torch.norm(C, p="fro")


# ----------------- Model -----------------
class MLPEncoder(nn.Module):
    def __init__(self, in_dim, z_dim=256, dropout=0.1):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(in_dim, 2048), nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(2048, 512), nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(512, z_dim)
        )

    def forward(self, x):
        return self.net(x)


class DenoisingAE(nn.Module):
    def __init__(self, in_dim, z_dim=256, dropout=0.1):
        super().__init__()
        self.enc = MLPEncoder(in_dim, z_dim, dropout)
        self.dec = nn.Sequential(
            nn.Linear(z_dim, 512), nn.ReLU(),
            nn.Linear(512, 2048), nn.ReLU(),
            nn.Linear(2048, in_dim)
        )

    def forward(self, x):
        z = self.enc(x)
        xhat = self.dec(z)
        return z, xhat


class CrossAttentionHead(nn.Module):
    def __init__(self, num_genes, h):
        super().__init__()
        self.val = nn.Linear(1, h)
        self.gene_id = nn.Embedding(num_genes, h)
        self.Wq = nn.Linear(h, h)
        self.Wk = nn.Linear(h, h)
        self.Wv = nn.Linear(h, h)

    def forward(self, x, z):
        B, N = x.shape
        device = x.device
        gene_ids = torch.arange(N, device=device).unsqueeze(0).expand(B, N)
        t = self.val(x.unsqueeze(-1)) + self.gene_id(gene_ids)  # [B,N,h]
        Q = self.Wq(z).unsqueeze(1)                             # [B,1,h]
        K = self.Wk(t)                                          # [B,N,h]
        V = self.Wv(t)                                          # [B,N,h]
        logits = torch.matmul(Q, K.transpose(1, 2)) / math.sqrt(K.size(-1))  # [B,1,N]
        w = torch.softmax(logits, dim=-1).squeeze(1)            # [B,N]
        pooled = torch.matmul(w.unsqueeze(1), V).squeeze(1)     # [B,h]
        return pooled, w


class FeatureDecomposer(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.to_inv = nn.Linear(dim, dim)
        self.to_bc  = nn.Linear(dim, dim)

    def forward(self, h):
        return self.to_inv(h), self.to_bc(h)


class UDAClassifierTwoHeads(nn.Module):
    def __init__(self, in_dim, num_genes, z_dim=256, dropout=0.1):
        super().__init__()
        self.encoder = MLPEncoder(in_dim, z_dim, dropout)
        self.attn = CrossAttentionHead(num_genes, z_dim)
        self.decomp = FeatureDecomposer(z_dim)

        # task head (v3: uses z_inv ONLY)
        self.head = nn.Sequential(
            nn.Linear(z_dim, z_dim // 2), nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(z_dim // 2, 2)
        )

        # platform head (RNAseq vs microarray) -> use z_bc
        self.platform = nn.Sequential(
            nn.Linear(z_dim, z_dim // 2), nn.ReLU(),
            nn.Linear(z_dim // 2, 2)
        )

        # cancer head (OTH vs TNBC) -> use z_inv
        self.cancer = nn.Sequential(
            nn.Linear(z_dim, z_dim // 2), nn.ReLU(),
            nn.Linear(z_dim // 2, 2)
        )

    def forward(self, x, grl_platform=None, grl_cancer=None):
        z_enc = self.encoder(x)
        pooled, w = self.attn(x, z_enc)
        z_inv, z_bc = self.decomp(pooled)

        # ✅ v3: task uses invariant part only
        logits = self.head(z_inv)
        z_all = z_inv

        plat_logits = None
        cancer_logits = None

        if grl_platform is not None:
            plat_logits = self.platform(grl(z_bc, grl_platform))
        if grl_cancer is not None:
            cancer_logits = self.cancer(grl(z_inv, grl_cancer))

        return logits, plat_logits, cancer_logits, z_all, w, pooled, z_inv, z_bc


# ----------------- Utils -----------------
def seed_all(sd=42):
    import random
    random.seed(sd)
    np.random.seed(sd)
    torch.manual_seed(sd)
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(sd)


def safe_torch_load(path, device):
    try:
        return torch.load(path, map_location=device, weights_only=True)
    except TypeError:
        return torch.load(path, map_location=device)


def _save_line_plot(values, ylabel, title, out_png):
    plt.figure()
    xs = np.arange(1, len(values) + 1)
    plt.plot(xs, values)
    plt.xlabel("Epoch")
    plt.ylabel(ylabel)
    plt.title(title)
    plt.tight_layout()
    plt.savefig(out_png, dpi=200)
    plt.close()


def _save_curve_csv(values, out_csv, colname):
    df = pd.DataFrame({"epoch": np.arange(1, len(values) + 1), colname: values})
    df.to_csv(out_csv, index=False)


def _save_roc(y_true, y_score, out_png, title="ROC Curve"):
    fpr, tpr, _ = roc_curve(y_true, y_score)
    plt.figure()
    if len(np.unique(y_true)) > 1:
        plt.plot(fpr, tpr, label=f"AUC={roc_auc_score(y_true, y_score):.4f}")
    else:
        plt.plot(fpr, tpr, label="AUC=nan")
    plt.plot([0, 1], [0, 1], "--")
    plt.xlabel("FPR")
    plt.ylabel("TPR")
    plt.title(title)
    plt.legend()
    plt.tight_layout()
    plt.savefig(out_png, dpi=200)
    plt.close()


def _bin_metrics(y_true, y_score, thr=0.5):
    y_pred = (y_score >= thr).astype(int)
    return {
        "AUC": float(roc_auc_score(y_true, y_score)) if len(np.unique(y_true)) > 1 else float("nan"),
        "ACC@0.5": float(accuracy_score(y_true, y_pred)),
        "F1@0.5": float(f1_score(y_true, y_pred, zero_division=0)),
        "N": int(len(y_true))
    }


@torch.no_grad()
def evaluate_auc(model, loader, device):
    model.eval()
    probs, labels, ids = [], [], []
    for batch in loader:
        xb, yb, sid, *_ = batch
        xb, yb = xb.to(device), yb.to(device)
        logits, _, _, _, _, _, _, _ = model(xb, grl_platform=None, grl_cancer=None)
        p = torch.softmax(logits, 1)[:, 1]
        probs.append(p.detach().cpu().numpy())
        labels.append(yb.detach().cpu().numpy())
        ids.extend(list(sid))
    y = np.concatenate(labels) if labels else np.array([])
    p = np.concatenate(probs) if probs else np.array([])
    auc = roc_auc_score(y, p) if (len(y) > 0 and len(np.unique(y)) > 1) else float("nan")
    return auc, p, y, ids


def as_long_label_batch(v, B: int, device):
    """Make label a LongTensor [B] on device."""
    if torch.is_tensor(v):
        v = v.to(device)
        if v.dtype != torch.long:
            v = v.long()
        if v.ndim == 0:
            return v.view(1).expand(B)
        if v.ndim == 1:
            if v.numel() == B:
                return v
            if v.numel() == 1:
                return v.expand(B)
        raise ValueError(f"Label tensor has unexpected shape: {tuple(v.shape)} (B={B})")
    return torch.full((B,), int(v), dtype=torch.long, device=device)


def read_genes_json(genes_json: Optional[str]):
    if not genes_json:
        return None
    obj = json.load(open(genes_json, "r", encoding="utf-8"))
    if isinstance(obj, dict) and "genes" in obj:
        return list(obj["genes"])
    if isinstance(obj, list):
        return list(obj)
    raise ValueError(f"genes.json format not recognized: {genes_json}")


def align_to_genes(df: pd.DataFrame, genes: List[str], label_col: Optional[str] = None):
    df = df.copy()
    keep_label = (label_col is not None and label_col in df.columns)

    cols = [c for c in df.columns if (c in genes) or (keep_label and c == label_col)]
    df = df[cols].copy()

    missing = [g for g in genes if g not in df.columns]
    for g in missing:
        df[g] = 0.0

    if keep_label:
        df = df[genes + [label_col]]
    else:
        df = df[genes]

    X = df[genes].apply(pd.to_numeric, errors="coerce").astype(np.float32).values
    n_nan = int(np.isnan(X).sum())
    n_inf = int(np.isinf(X).sum())
    if n_nan or n_inf:
        print(f"[WARN] NaN={n_nan}, Inf={n_inf} -> impute NaN by col mean, Inf->NaN")
        X = np.where(np.isinf(X), np.nan, X)
        col_mean = np.nanmean(X, axis=0)
        idx = np.where(np.isnan(X))
        X[idx] = np.take(col_mean, idx[1])
        df[genes] = X
    return df


# ----------------- Pseudo-label selection -----------------
def pick_balanced_pseudolabels(pt, tau, max_per_class=8, min_per_class=2):
    conf, pseudo = pt.max(dim=1)
    base = conf >= tau

    B = pt.size(0)
    m = torch.zeros(B, dtype=torch.bool, device=pt.device)

    for cls in [0, 1]:
        idx = torch.where(base & (pseudo == cls))[0]
        if idx.numel() > 0:
            order = torch.argsort(conf[idx], descending=True)
            keep = idx[order[:max_per_class]]
            m[keep] = True

    for cls in [0, 1]:
        has_cls = int(((pseudo[m] == cls).sum().item())) if m.any() else 0
        if has_cls == 0:
            un = torch.where(~m)[0]
            if un.numel() > 0:
                scores = pt[un, cls]
                order = torch.argsort(scores, descending=True)
                take = un[order[:min_per_class]]
                m[take] = True
                pseudo[take] = cls

    if m.sum() == 0:
        k = min(max(1, B // 4), B)
        order = torch.argsort(conf, descending=True)
        m[order[:k]] = True

    mask_rate = float(m.float().mean().item())
    pos_rate  = float((pseudo[m] == 1).float().mean().item()) if m.any() else 0.0
    return m, pseudo, conf, mask_rate, pos_rate


# ----------------- schedule helpers -----------------
def ramp(p: float, warm: float = 0.2) -> float:
    """0..warm => 0; warm..1 => linear to 1"""
    if p < warm:
        return 0.0
    if warm >= 1.0:
        return 1.0
    return float(min(1.0, (p - warm) / (1.0 - warm)))


def cycle_loader(loader):
    while True:
        for b in loader:
            yield b


def make_loader(ds, batch, shuffle, drop_last, num_workers=0):
    return DataLoader(ds, batch_size=batch, shuffle=shuffle, drop_last=drop_last, num_workers=num_workers)


# ----------------- Stage0: global AE (unlabeled-only) -----------------
def run_global_ae_pretrain(cfg, out_root: Path, device, in_dim, gene_list,
                          oth_tr: pd.DataFrame, tnbc_arr_ul: pd.DataFrame, tnbc_rna_ul: Optional[pd.DataFrame]):
    if cfg["pretrain_epochs"] <= 0:
        return None

    enc_path = out_root / "encoder_pretrained.pt"
    if cfg.get("reuse_global_pretrain", True) and enc_path.exists():
        print("[Stage0] Found existing encoder_pretrained.pt -> reuse.")
        return enc_path

    pool = [oth_tr[gene_list].values.astype(np.float32),
            tnbc_arr_ul[gene_list].values.astype(np.float32)]
    if tnbc_rna_ul is not None:
        pool.append(tnbc_rna_ul[gene_list].values.astype(np.float32))

    X_pool = np.concatenate(pool, axis=0)
    print(f"[AE] global pool size={X_pool.shape[0]} (OTH_tr + TNBC_arr_ul + optional TNBC_rna_ul)")

    ae = DenoisingAE(in_dim, cfg["z_dim"], cfg["dropout"]).to(device)
    opt_ae = torch.optim.Adam(ae.parameters(), lr=cfg["lr_pre"])
    ae.train()

    dl_pool = make_loader(TensorOnlyDS(X_pool), cfg["batch"], shuffle=True,
                          drop_last=(X_pool.shape[0] >= cfg["batch"]), num_workers=cfg["num_workers"])

    mse_log = []
    for ep in range(cfg["pretrain_epochs"]):
        losses = []
        for xb in dl_pool:
            xb = xb.to(device)
            mask = (torch.rand_like(xb) > cfg["mask_ratio"]).float()
            noisy = xb * mask + torch.randn_like(xb) * cfg["noise_std"]
            _, xhat = ae(noisy)
            loss = F.mse_loss(xhat, xb)

            opt_ae.zero_grad()
            loss.backward()
            opt_ae.step()
            losses.append(loss.item())

        ep_mse = float(np.mean(losses)) if losses else float("nan")
        mse_log.append(ep_mse)
        print(f"[Pretrain] epoch {ep} reconMSE={ep_mse:.4f}")

    torch.save(ae.enc.state_dict(), enc_path)
    _save_curve_csv(mse_log, out_root / "pretrain_reconMSE.csv", "reconMSE")
    _save_line_plot(mse_log, "recon MSE", "Global Pretrain Reconstruction MSE", out_root / "pretrain_reconMSE.png")
    del ae
    return enc_path


# ----------------- Stage1: global source supervised (OTH) -----------------
def run_global_stage1(cfg, out_root: Path, device, in_dim, gene_list, trL, vaL, enc_path: Optional[Path]):
    stage1_shared = out_root / "stage1b_best_shared.pt"
    if cfg.get("reuse_global_stage1", True) and stage1_shared.exists():
        print("[Stage1] Found existing stage1b_best_shared.pt -> reuse.")
        return stage1_shared

    model = UDAClassifierTwoHeads(in_dim, num_genes=in_dim, z_dim=cfg["z_dim"], dropout=cfg["dropout"]).to(device)
    if enc_path is not None and Path(enc_path).exists():
        model.encoder.load_state_dict(safe_torch_load(enc_path, device), strict=True)
        print("[Stage0->1] Loaded pretrained encoder into model.")

    focal = FocalLoss(cfg["focal_gamma"], cfg["focal_alpha"])
    center = CenterLoss(2, cfg["z_dim"]).to(device)

    # Stage1a freeze encoder
    for p in model.encoder.parameters():
        p.requires_grad = False

    opt = torch.optim.Adam(
        [
            {"params": [p for n, p in model.named_parameters() if p.requires_grad]},
            {"params": center.parameters(), "lr": cfg["lr"] * 0.5},
        ],
        lr=cfg["lr"],
        weight_decay=cfg["weight_decay"],
    )

    best_auc, bad = -1, 0
    auc_log = []
    for ep in range(cfg["src_head_epochs"]):
        model.train()
        for xb, yb, *_ in trL:
            xb, yb = xb.to(device), yb.to(device)
            logits, _, _, z_all, _, _, z_inv, z_bc = model(xb, None, None)
            L_cls = focal(logits, yb) + cfg["lambda_center"] * center(z_all, yb)
            L_ort = orth_loss(z_inv, z_bc) * cfg["lambda_ortho"]
            loss = L_cls + L_ort

            opt.zero_grad()
            loss.backward()
            if cfg["grad_clip"] > 0:
                torch.nn.utils.clip_grad_norm_(model.parameters(), cfg["grad_clip"])
            opt.step()

        auc, _, _, _ = evaluate_auc(model, vaL, device)
        auc_log.append(float(auc))
        print(f"[Stage1a] epoch {ep} OTH-val AUC={auc:.4f}")
        if auc > best_auc:
            best_auc, bad = auc, 0
            torch.save(model.state_dict(), out_root / "stage1a_best_shared.pt")
        else:
            bad += 1
        if bad >= cfg["patience"]:
            break

    if auc_log:
        _save_curve_csv(auc_log, out_root / "stage1a_auc.csv", "AUC")
        _save_line_plot(auc_log, "AUC", "Stage1a OTH-val AUC (shared)", out_root / "stage1a_auc.png")

    # Stage1b unfreeze last layers lightly
    model.load_state_dict(safe_torch_load(out_root / "stage1a_best_shared.pt", device), strict=True)
    for p in model.encoder.parameters():
        p.requires_grad = False
    for n, p in model.encoder.named_parameters():
        if ("net.3" in n) or ("net.6" in n):
            p.requires_grad = True

    opt = torch.optim.Adam(
        [
            {"params": [p for n, p in model.named_parameters() if p.requires_grad]},
            {"params": center.parameters(), "lr": cfg["lr"] * 0.5},
        ],
        lr=cfg["lr"] * 0.3,
        weight_decay=cfg["weight_decay"],
    )

    best_auc, bad = -1, 0
    auc_log = []
    for ep in range(cfg["src_ft_epochs"]):
        model.train()
        for xb, yb, *_ in trL:
            xb, yb = xb.to(device), yb.to(device)
            logits, _, _, z_all, _, _, z_inv, z_bc = model(xb, None, None)
            L_cls = focal(logits, yb) + cfg["lambda_center"] * center(z_all, yb)
            L_ort = orth_loss(z_inv, z_bc) * cfg["lambda_ortho"]
            loss = L_cls + L_ort

            opt.zero_grad()
            loss.backward()
            if cfg["grad_clip"] > 0:
                torch.nn.utils.clip_grad_norm_(model.parameters(), cfg["grad_clip"])
            opt.step()

        auc, _, _, _ = evaluate_auc(model, vaL, device)
        auc_log.append(float(auc))
        print(f"[Stage1b] epoch {ep} OTH-val AUC={auc:.4f}")
        if auc > best_auc:
            best_auc, bad = auc, 0
            torch.save(model.state_dict(), stage1_shared)
        else:
            bad += 1
        if bad >= cfg["patience"]:
            break

    if auc_log:
        _save_curve_csv(auc_log, out_root / "stage1b_auc.csv", "AUC")
        _save_line_plot(auc_log, "AUC", "Stage1b OTH-val AUC (shared)", out_root / "stage1b_auc.png")

    return stage1_shared


# ----------------- Stage2: per-fold training + outer-test eval -----------------
def run_fold_stage2(cfg, fold_dir: Path, device, gene_list, in_dim,
                    trL, vaL, tnbcArrUL_loader, tnbcRnaUL_loader,
                    df_tlab_train: pd.DataFrame, df_tlab_val: pd.DataFrame, df_test: pd.DataFrame,
                    init_state_path: Path):

    # labels (fixed by file identity)
    PLAT_RNASEQ = 0
    PLAT_ARRAY  = 1
    CANC_OTH  = 0
    CANC_TNBC = 1

    # loaders for target labeled: inner train / inner val / outer test
    def dl_labeled(df, shuffle, drop_last):
        return make_loader(
            LabeledDS(df, gene_list, cfg["label_col"], platform_id=PLAT_ARRAY, cancer_id=CANC_TNBC),
            cfg["batch"], shuffle=shuffle, drop_last=drop_last, num_workers=cfg["num_workers"]
        )

    tlab_train = dl_labeled(df_tlab_train, shuffle=True, drop_last=(len(df_tlab_train) >= cfg["batch"]))
    tlab_val   = dl_labeled(df_tlab_val,   shuffle=False, drop_last=False)
    tlab_test  = dl_labeled(df_test,       shuffle=False, drop_last=False)

    # model init
    model = UDAClassifierTwoHeads(in_dim, num_genes=in_dim, z_dim=cfg["z_dim"], dropout=cfg["dropout"]).to(device)
    model.load_state_dict(safe_torch_load(init_state_path, device), strict=True)

    focal = FocalLoss(cfg["focal_gamma"], cfg["focal_alpha"])
    center = CenterLoss(2, cfg["z_dim"]).to(device)

    it_src  = cycle_loader(trL)
    it_arr  = cycle_loader(tnbcArrUL_loader)
    it_rna  = cycle_loader(tnbcRnaUL_loader) if tnbcRnaUL_loader is not None else None
    it_tlab = cycle_loader(tlab_train)

    # steps per epoch
    steps_per_epoch = cfg["steps_per_epoch"]
    if steps_per_epoch <= 0:
        steps_per_epoch = min(len(trL), len(tnbcArrUL_loader))
        if tnbcRnaUL_loader is not None:
            steps_per_epoch = min(steps_per_epoch, len(tnbcRnaUL_loader))
        steps_per_epoch = max(1, steps_per_epoch)

    total_steps = max(1, steps_per_epoch * cfg["uda_epochs"])
    gstep = 0

    opt = torch.optim.Adam(
        [
            {"params": model.parameters()},
            {"params": center.parameters(), "lr": cfg["lr"] * 0.5},
        ],
        lr=cfg["lr"],
        weight_decay=cfg["weight_decay"],
    )

    best_oth_auc = -1.0
    best_oth_epoch = -1
    best_tval_auc = -1.0
    best_tval_epoch = -1

    auc_log_oth = []
    auc_log_tval = []
    tau_last = None

    warned_platform_fallback = False
    warned_cancer_disabled = False

    # early stop on target-val (optional)
    bad_t = 0

    for ep in range(cfg["uda_epochs"]):
        model.train()
        ep_mask_rate = []
        ep_pos_rate = []

        for _ in range(steps_per_epoch):
            prog = gstep / (total_steps - 1) if total_steps > 1 else 1.0
            r_all = ramp(prog, warm=cfg["warmup"])
            grl_plat = cfg["lambda_platform"] * r_all
            grl_canc = cfg["lambda_cancer"] * r_all

            # source batch (OTH RNAseq)
            xb_s, yb_s, _, plat_s, canc_s = next(it_src)
            xb_s, yb_s = xb_s.to(device), yb_s.to(device)
            plat_s = as_long_label_batch(plat_s, xb_s.size(0), device)
            canc_s = as_long_label_batch(canc_s, xb_s.size(0), device)

            # target microarray batch (TNBC array unlabeled)
            xb_a, _, plat_a, canc_a = next(it_arr)
            xb_a = xb_a.to(device)
            plat_a = as_long_label_batch(plat_a, xb_a.size(0), device)
            canc_a = as_long_label_batch(canc_a, xb_a.size(0), device)

            # optional TNBC RNAseq batch
            xb_r = None
            plat_r = None
            canc_r = None
            if it_rna is not None:
                xb_r, _, plat_r0, canc_r0 = next(it_rna)
                xb_r = xb_r.to(device)
                plat_r = as_long_label_batch(plat_r0, xb_r.size(0), device)
                canc_r = as_long_label_batch(canc_r0, xb_r.size(0), device)

            # ---- forward ----
            logits_s, platlog_s, canclog_s, z_all_s, _, _, z_inv_s, z_bc_s = model(
                xb_s, grl_platform=grl_plat, grl_cancer=grl_canc
            )
            logits_a, platlog_a, canclog_a, z_all_a, _, _, z_inv_a, z_bc_a = model(
                xb_a, grl_platform=grl_plat, grl_cancer=grl_canc
            )
            if xb_r is not None:
                logits_r, platlog_r, canclog_r, z_all_r, _, _, z_inv_r, z_bc_r = model(
                    xb_r, grl_platform=grl_plat, grl_cancer=grl_canc
                )

            # ---- source task loss ----
            L_src = focal(logits_s, yb_s) + cfg["lambda_center"] * center(z_all_s, yb_s)

            # ---- SSDA target anchor (inner train) ----
            xb_t, yb_t, *_ = next(it_tlab)
            xb_t, yb_t = xb_t.to(device), yb_t.to(device)
            logits_t, _, _, z_all_t, _, _, z_inv_t, z_bc_t = model(xb_t, None, None)
            L_tlab = (focal(logits_t, yb_t) + cfg["lambda_center"] * center(z_all_t, yb_t)) * cfg["lambda_tlab"]

            # ---- pseudo-label on microarray unlabeled ----
            pt_a = torch.softmax(logits_a, 1)
            tau = cfg["pl_tau_start"] + (cfg["pl_tau_end"] - cfg["pl_tau_start"]) * prog
            tau_last = float(tau)

            mask_pl, pseudo, conf, mask_rate, pos_rate = pick_balanced_pseudolabels(
                pt_a, tau=tau,
                max_per_class=cfg["pl_max_per_class"],
                min_per_class=cfg["pl_min_per_class"]
            )
            ep_mask_rate.append(mask_rate)
            ep_pos_rate.append(pos_rate)

            # entropy minimization (scheduled)
            if cfg["lambda_entropy"] > 0:
                if cfg["ent_on_conf"] and mask_pl.any():
                    pt_sel = pt_a[mask_pl]
                else:
                    pt_sel = pt_a
                L_ent = (-(pt_sel * (pt_sel.clamp_min(1e-8)).log()).sum(dim=1)).mean()
                L_ent = L_ent * (cfg["lambda_entropy"] * r_all)
            else:
                L_ent = 0.0

            # pseudo-label CE (scheduled)
            if cfg["lambda_pl"] > 0 and mask_pl.any():
                L_pl = F.cross_entropy(logits_a[mask_pl], pseudo[mask_pl]) * (cfg["lambda_pl"] * r_all)
            else:
                L_pl = 0.0

            # ---- platform adversarial loss ----
            L_plat = 0.0
            if cfg["lambda_platform_loss"] > 0 and r_all > 0:
                if xb_r is not None and cfg["platform_within_tnbc"]:
                    L_plat = F.cross_entropy(platlog_a, plat_a) + F.cross_entropy(platlog_r, plat_r)
                else:
                    if cfg["fallback_use_oth_for_platform_if_no_tnbc_rna"]:
                        if not warned_platform_fallback:
                            print("[WARN] No TNBC RNA UL for within-TNBC platform loss -> fallback to (OTH RNA) vs (TNBC array). "
                                  "This may re-introduce platform/cancer collinearity.")
                            warned_platform_fallback = True
                        L_plat = F.cross_entropy(platlog_a, plat_a) + F.cross_entropy(platlog_s, plat_s)
                    else:
                        L_plat = 0.0
                L_plat = L_plat * (cfg["lambda_platform_loss"] * r_all)

            # ---- cancer adversarial loss ----
            L_canc = 0.0
            if cfg["lambda_cancer_loss"] > 0 and r_all > 0:
                if xb_r is not None and cfg["cancer_within_rnaseq"]:
                    L_canc = F.cross_entropy(canclog_s, canc_s) + F.cross_entropy(canclog_r, canc_r)
                    L_canc = L_canc * (cfg["lambda_cancer_loss"] * r_all)
                else:
                    if cfg["disable_cancer_if_no_tnbc_rna"]:
                        if not warned_cancer_disabled:
                            print("[WARN] No TNBC RNA UL -> cancer loss disabled (to avoid cancer/platform collinearity via microarray).")
                            warned_cancer_disabled = True
                        L_canc = 0.0
                    else:
                        L_canc = (F.cross_entropy(canclog_s, canc_s) + F.cross_entropy(canclog_a, canc_a)) * (cfg["lambda_cancer_loss"] * r_all)

            # ---- orth regularization ----
            L_ortho = 0.5 * cfg["lambda_ortho"] * (orth_loss(z_inv_s, z_bc_s) + orth_loss(z_inv_a, z_bc_a))
            if xb_r is not None:
                L_ortho = L_ortho + (cfg["lambda_ortho"] * 0.25 * orth_loss(z_inv_r, z_bc_r))

            loss = L_src + L_tlab + L_ent + L_pl + L_plat + L_canc + L_ortho

            opt.zero_grad()
            loss.backward()
            if cfg["grad_clip"] > 0:
                torch.nn.utils.clip_grad_norm_(model.parameters(), cfg["grad_clip"])
            opt.step()
            gstep += 1

        # ---- monitor ----
        auc_oth, p_oth, y_oth, ids_oth = evaluate_auc(model, vaL, device)
        auc_log_oth.append(float(auc_oth))

        auc_tval, p_tval, y_tval, ids_tval = evaluate_auc(model, tlab_val, device)
        auc_log_tval.append(float(auc_tval))

        mr = float(np.mean(ep_mask_rate)) if ep_mask_rate else 0.0
        pr = float(np.mean(ep_pos_rate)) if ep_pos_rate else 0.0
        print(f"[Fold-UDA] epoch {ep} OTH-val AUC={auc_oth:.4f} | TNBC-inner-val AUC={auc_tval:.4f} "
              f"(tau~{tau_last:.2f}) warm_r={ramp((gstep-1)/max(1,total_steps-1), cfg['warmup']):.2f} mask={mr:.3f} mask&1={pr:.3f}")

        # save best by oth-val
        if float(auc_oth) > best_oth_auc:
            best_oth_auc = float(auc_oth)
            best_oth_epoch = ep + 1
            torch.save(model.state_dict(), fold_dir / "checkpoint_uda_best_othval.pt")

        # save best by inner target-val (THIS is the one used for outer-test)
        if (not np.isnan(auc_tval)) and float(auc_tval) > best_tval_auc:
            best_tval_auc = float(auc_tval)
            best_tval_epoch = ep + 1
            bad_t = 0
            torch.save(model.state_dict(), fold_dir / "checkpoint_uda_best_target.pt")
        else:
            bad_t += 1

        if cfg.get("uda_patience", 0) > 0 and bad_t >= cfg["uda_patience"]:
            print(f"[Fold-UDA] early stop on target-val: bad_t={bad_t} >= patience={cfg['uda_patience']}")
            break

    # save curves
    if auc_log_oth:
        _save_curve_csv(auc_log_oth, fold_dir / "uda_auc_othval.csv", "AUC")
        _save_line_plot(auc_log_oth, "AUC", "UDA OTH-val AUC", fold_dir / "uda_auc_othval.png")
    if auc_log_tval:
        _save_curve_csv(auc_log_tval, fold_dir / "uda_auc_tnbc_inner_val.csv", "AUC")
        _save_line_plot(auc_log_tval, "AUC", "UDA TNBC inner-val AUC", fold_dir / "uda_auc_tnbc_inner_val.png")

    # load best_target for final evals
    ckpt_t = fold_dir / "checkpoint_uda_best_target.pt"
    if ckpt_t.exists():
        model.load_state_dict(safe_torch_load(ckpt_t, device), strict=True)
        chosen = "best_target"
    else:
        chosen = "last"
    print(f"[Fold] Using checkpoint for outer-test: {chosen}")

    # eval & export: OTH-val / inner-val / outer-test
    def export_split(tag, loader):
        auc, p, y, ids = evaluate_auc(model, loader, device)
        pd.DataFrame({"ID": ids, "Prob": p, "True": y}).to_csv(fold_dir / f"pred_{tag}.csv", index=False)
        _save_roc(y, p, fold_dir / f"roc_{tag}.png", title=f"ROC ({tag})")
        met = _bin_metrics(y, p, thr=0.5)
        pd.DataFrame([met]).to_csv(fold_dir / f"metrics_{tag}.csv", index=False)
        return auc, met

    auc_oth2, met_oth = export_split("oth_val", vaL)
    auc_tval2, met_tval = export_split("tnbc_inner_val", tlab_val)
    auc_test, met_test = export_split("tnbc_outer_test", tlab_test)

    summary = {
        "chosen": chosen,
        "best_oth_epoch": int(best_oth_epoch),
        "best_oth_auc": float(best_oth_auc),
        "best_tnbc_inner_val_epoch": int(best_tval_epoch),
        "best_tnbc_inner_val_auc": float(best_tval_auc),
        "final_auc_oth_val": float(auc_oth2),
        "final_auc_tnbc_inner_val": float(auc_tval2),
        "final_auc_tnbc_outer_test": float(auc_test),
        "OTH_val": met_oth,
        "TNBC_inner_val": met_tval,
        "TNBC_outer_test": met_test,
    }
    with open(fold_dir / "metrics_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)

    return summary


# ----------------- Main -----------------
def main():
    cfg = CFG
    seed_all(cfg["seed"])
    device = torch.device("cuda" if torch.cuda.is_available() and (not cfg["cpu"]) else "cpu")

    out_root = Path(cfg["out_dir"])
    out_root.mkdir(parents=True, exist_ok=True)

    # ---- load data ----
    oth = pd.read_csv(cfg["oth_labeled_csv"], index_col=0)
    tnbc_arr_ul = pd.read_csv(cfg["tnbc_array_unlabeled_csv"], index_col=0)

    tnbc_rna_ul = None
    if cfg["tnbc_rnaseq_unlabeled_csv"]:
        tnbc_rna_ul = pd.read_csv(cfg["tnbc_rnaseq_unlabeled_csv"], index_col=0)

    tnbc_arr_lab = pd.read_csv(cfg["tnbc_array_labeled_csv"], index_col=0)

    # ---- decide gene list (global, used for all folds) ----
    genes_json_list = read_genes_json(cfg["genes_json"]) if cfg["genes_json"] else None

    if genes_json_list is not None:
        gene_list = [g for g in genes_json_list if g in oth.columns and g in tnbc_arr_ul.columns and g in tnbc_arr_lab.columns]
        if tnbc_rna_ul is not None:
            gene_list = [g for g in gene_list if g in tnbc_rna_ul.columns]
        print(f"[GENES] from genes.json -> after intersection = {len(gene_list)}")
    else:
        gene_list = [g for g in oth.columns if g != cfg["label_col"]
                     and g in tnbc_arr_ul.columns and g in tnbc_arr_lab.columns
                     and (tnbc_rna_ul is None or g in tnbc_rna_ul.columns)]
        print(f"[GENES] intersection from csvs = {len(gene_list)}")

    assert len(gene_list) > 0, "No intersected genes found."

    # align all to same gene_list
    oth = align_to_genes(oth, gene_list, label_col=cfg["label_col"])
    tnbc_arr_ul = align_to_genes(tnbc_arr_ul, gene_list, label_col=None)
    if tnbc_rna_ul is not None:
        tnbc_rna_ul = align_to_genes(tnbc_rna_ul, gene_list, label_col=None)
    tnbc_arr_lab = align_to_genes(tnbc_arr_lab, gene_list, label_col=cfg["label_col"])

    in_dim = len(gene_list)
    print(f"[DATA] genes={in_dim} | OTH={len(oth)} | TNBC_arr_ul={len(tnbc_arr_ul)} | "
          f"TNBC_rna_ul={(len(tnbc_rna_ul) if tnbc_rna_ul is not None else 0)} | TNBC_arr_lab={len(tnbc_arr_lab)}")

    # save genes.json (this run)
    with open(out_root / "genes.json", "w", encoding="utf-8") as f:
        json.dump({"genes": gene_list}, f, ensure_ascii=False, indent=2)

    # labels (fixed by file identity)
    PLAT_RNASEQ = 0
    PLAT_ARRAY  = 1
    CANC_OTH  = 0
    CANC_TNBC = 1

    # =========================
    # Build OTH loaders (STRATIFIED splits) (shared across folds)
    # =========================
    y_oth = oth[cfg["label_col"]].astype(int).values
    idx_all = np.arange(len(oth))
    tr_idx, va_idx = train_test_split(
        idx_all, test_size=cfg["val_ratio"], random_state=cfg["seed"],
        stratify=y_oth if len(np.unique(y_oth)) == 2 else None
    )
    oth_tr = oth.iloc[tr_idx].copy()
    oth_va = oth.iloc[va_idx].copy()

    trL = make_loader(
        LabeledDS(oth_tr, gene_list, cfg["label_col"], platform_id=PLAT_RNASEQ, cancer_id=CANC_OTH),
        cfg["batch"], shuffle=True, drop_last=(len(oth_tr) >= cfg["batch"]), num_workers=cfg["num_workers"]
    )
    vaL = make_loader(
        LabeledDS(oth_va, gene_list, cfg["label_col"], platform_id=PLAT_RNASEQ, cancer_id=CANC_OTH),
        cfg["batch"], shuffle=False, drop_last=False, num_workers=cfg["num_workers"]
    )

    # TNBC unlabeled loaders (shared across folds)
    tnbcArrUL = make_loader(
        UnlabeledDS(tnbc_arr_ul, gene_list, platform_id=PLAT_ARRAY, cancer_id=CANC_TNBC),
        cfg["batch"], shuffle=True, drop_last=(len(tnbc_arr_ul) >= cfg["batch"]), num_workers=cfg["num_workers"]
    )

    tnbcRnaUL = None
    if tnbc_rna_ul is not None:
        tnbcRnaUL = make_loader(
            UnlabeledDS(tnbc_rna_ul, gene_list, platform_id=PLAT_RNASEQ, cancer_id=CANC_TNBC),
            cfg["batch"], shuffle=True, drop_last=(len(tnbc_rna_ul) >= cfg["batch"]), num_workers=cfg["num_workers"]
        )

    # =========================
    # Global Stage0 (AE, unlabeled-only, once)
    # =========================
    enc_path = None
    if cfg["pretrain_epochs"] > 0:
        enc_path = run_global_ae_pretrain(cfg, out_root, device, in_dim, gene_list, oth_tr, tnbc_arr_ul, tnbc_rna_ul)

    # =========================
    # Global Stage1 (OTH supervised, once)
    # =========================
    stage1_shared = run_global_stage1(cfg, out_root, device, in_dim, gene_list, trL, vaL, enc_path)

    # =========================
    # Outer 5-fold CV on TNBC labeled
    # =========================
    y_tnbc = tnbc_arr_lab[cfg["label_col"]].astype(int).values
    idx = np.arange(len(tnbc_arr_lab))
    skf = StratifiedKFold(n_splits=cfg["outer_folds"], shuffle=True, random_state=cfg["seed"])

    cv_rows = []
    for fold, (trval_idx, test_idx) in enumerate(skf.split(idx, y_tnbc), 1):
        fold_dir = out_root / f"fold{fold}"
        fold_dir.mkdir(parents=True, exist_ok=True)

        df_trval = tnbc_arr_lab.iloc[trval_idx].copy()
        df_test  = tnbc_arr_lab.iloc[test_idx].copy()

        # inner split: train / val (for checkpoint selection)
        y_trval = df_trval[cfg["label_col"]].astype(int).values
        idx_trval = np.arange(len(df_trval))
        tr_i, va_i = train_test_split(
            idx_trval, test_size=cfg["tnbc_lab_val_ratio"],
            random_state=cfg["seed"] + 1000 + fold,
            stratify=y_trval if len(np.unique(y_trval)) == 2 else None
        )
        df_tlab_train = df_trval.iloc[tr_i].copy()
        df_tlab_val   = df_trval.iloc[va_i].copy()

        print(f"\n===== [Fold {fold}/{cfg['outer_folds']}] "
              f"trval={len(df_trval)} (inner-tr={len(df_tlab_train)}, inner-val={len(df_tlab_val)}) | test={len(df_test)} =====")

        # fold-specific seed (stability across folds)
        seed_all(cfg["seed"] + fold)

        summary = run_fold_stage2(
            cfg, fold_dir, device, gene_list, in_dim,
            trL, vaL, tnbcArrUL, tnbcRnaUL,
            df_tlab_train, df_tlab_val, df_test,
            init_state_path=stage1_shared
        )

        cv_rows.append({
            "fold": fold,
            "outer_test_auc": summary.get("final_auc_tnbc_outer_test", float("nan")),
            "inner_val_auc_best": summary.get("best_tnbc_inner_val_auc", float("nan")),
            "oth_val_auc_best": summary.get("best_oth_auc", float("nan")),
            "chosen": summary.get("chosen", ""),
            "best_epoch_inner_val": summary.get("best_tnbc_inner_val_epoch", -1),
        })

    cv_df = pd.DataFrame(cv_rows)
    cv_df.to_csv(out_root / "cv_summary.csv", index=False)

    mean_auc = float(np.nanmean(cv_df["outer_test_auc"].values)) if len(cv_df) else float("nan")
    std_auc  = float(np.nanstd(cv_df["outer_test_auc"].values)) if len(cv_df) else float("nan")
    print("\n====================")
    print("[CV] outer-test AUC per fold:")
    print(cv_df[["fold", "outer_test_auc", "best_epoch_inner_val"]])
    print(f"[CV] mean±std = {mean_auc:.4f} ± {std_auc:.4f}")
    print("[DONE] artifacts at:", out_root)


# =========================
# ✅ 只改这里！点 Run！
# =========================
CFG = dict(
    # ---- Paths ----
    oth_labeled_csv=r"D:\work1\task 3\02 grl\grl_1\std\OTH_labeled_std.csv",
    tnbc_array_unlabeled_csv=r"D:\work1\task 3\02 grl\grl_1\std\BRA_unlabeled_std.csv",
    tnbc_rnaseq_unlabeled_csv=r"D:\work1\task 3\02 grl\grl_1\std\TNBC_ULR_std.csv",
    tnbc_array_labeled_csv=r"D:\work1\task 3\02 grl\grl_1\std\TNBC_labeled_std.csv",
    label_col="Response",

    # optional genes.json (recommended)
    genes_json=r"D:\work1\task 3\02 grl\grl_1\std\outint_v3\genes.json",

    # outputs (root; folds inside)
    out_dir=r"D:\work1\task 3\02 grl\grl_1\std\outint_v3_cv5",

    # device
    cpu=True,
    num_workers=0,   # Windows 通常用 0 最稳

    # model/training
    z_dim=256,
    dropout=0.1,
    batch=32,
    val_ratio=0.2,      # OTH stratified split
    seed=42,

    # regularization
    weight_decay=1e-4,
    grad_clip=5.0,

    # Stage0 (DAE) [global once, unlabeled-only]
    pretrain_epochs=50,
    lr_pre=1e-3,
    mask_ratio=0.1,
    noise_std=0.05,
    reuse_global_pretrain=True,

    # Stage1 (global once)
    reuse_global_stage1=True,
    src_head_epochs=10,
    src_ft_epochs=30,
    lr=1e-4,
    patience=5,
    focal_gamma=2.0,
    focal_alpha=0.25,
    lambda_center=0.1,
    lambda_ortho=0.01,

    # Outer CV
    outer_folds=5,
    tnbc_lab_val_ratio=0.2,   # inner-val ratio inside trval

    # Stage2 (per fold)
    uda_epochs=30,
    steps_per_epoch=0,  # 0=auto
    uda_patience=0,     # 0=disable; e.g. 20 to early-stop on target inner-val

    # warmup
    warmup=0.2,

    # GRL strengths (scheduled by warmup)
    lambda_platform=0.10,
    lambda_cancer=0.03,

    # loss weights (scheduled by warmup)
    lambda_platform_loss=1.0,
    lambda_cancer_loss=0.3,

    # SSDA target anchor (inner train)
    lambda_tlab=1.0,

    # pseudo-label/entropy on microarray target (scheduled by warmup)
    lambda_entropy=0.01,
    lambda_pl=0.2,
    pl_tau_start=0.95,
    pl_tau_end=0.80,
    pl_max_per_class=8,
    pl_min_per_class=2,
    ent_on_conf=True,

    # de-collinearity switches
    platform_within_tnbc=True,
    cancer_within_rnaseq=True,
    fallback_use_oth_for_platform_if_no_tnbc_rna=True,
    disable_cancer_if_no_tnbc_rna=True,
)

if __name__ == "__main__":
    main()
