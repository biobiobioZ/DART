...
def train_stage2_tnbc_Aplus(
    ...
    for ep in range(epochs):
        ...
        thr_ep = best_threshold_by_f1(yv, pv)
        pred_ep = (pv >= thr_ep).astype(int)
        f1_ep = float(f1_score(yv, pred_ep, zero_division=0))
        acc_ep = float(accuracy_score(yv, pred_ep))

        is_best = int(score > best_score)
        ...
        if is_best:
            ...
            best_log = {
                ...
                "tnbc_val_thr": float(thr_ep),
                "tnbc_val_f1": float(f1_ep),
                "tnbc_val_acc": float(acc_ep),
        ...
        history_rows.append({
            "epoch": int(ep),
            "phase": int(1 if ep < phase1_epochs else 2),
            "train_sup_loss": float(np.mean(losses)) if len(losses) else np.nan,
            "train_l2sp": float(np.mean(l2sps)) if len(l2sps) else np.nan,
            "tnbc_val_auc": float(auc_tnbc),
            "oth_val_auc": float(auc_oth),
            "tnbc_val_thr": float(thr_ep),
            "tnbc_val_f1": float(f1_ep),
            ...
            "is_best": int(is_best),
            "lr_head": float(opt.param_groups[0]["lr"]),
            "lr_enc": float(opt.param_groups[1]["lr"]) if len(opt.param_groups) > 1 else 0.0,
            "lambda_l2sp": float(lambda_l2sp if ep >= phase1_epochs else 0.0),
            "use_mixup": int(bool(use_mixup)),
            "mixup_alpha": float(mixup_alpha),
        })

    hist_path = out_dir / "stage2_history.csv"
    ...
    best_log["stage2_history_csv"] = str(hist_path)
