...
def train_stage1_oth_supervised(
    ...
    out_dir: Path,
    seed: int,
    stage0_encoder_ckpt: Optional[str],
    z_dim: int,
    enc_hidden: int,
    clf_hidden: int,
    dropout: float,
...
def train_stage1_oth_supervised_grl_v2(
    ...
    if stage0_encoder_ckpt:
        ...
        print("[Stage1-GRLv2] stage0 disabled -> random init encoder")
...
def train_stage2_tnbc_Aplus(
    ...
    stage2_unfreeze_lastlayer: bool,
    stage2_phase1_encoder_eval: bool,
    ...
    if not bool(stage2_unfreeze_lastlayer):
        # keep encoder frozen for all epochs
        phase1_epochs = epochs
        print(f"[Stage2] Phase1(head-only)={phase1_epochs} epochs (encoder frozen for all epochs; no Phase2)")
    else:
        print(f"[Stage2] Phase1(head-only)={phase1_epochs} epochs, Phase2(unfreeze lastlayer + L2SP)={epochs-phase1_epochs} epochs")

    # Phase1: freeze encoder; optionally call enc.eval() to disable Dropout noise
    freeze_all_encoder(enc)
    if bool(stage2_phase1_encoder_eval):
        enc.eval()
    else:
        enc.train()  # encoder frozen but keep dropout noise (ablation)
    clf.train()
    ...
    for ep in range(epochs):
        # switch to phase2
        if bool(stage2_unfreeze_lastlayer) and ep == phase1_epochs:
...
def main(CFG: Dict):
    ...
    # Stage0 once
    stage0_path: Optional[str] = None
    if bool(CFG.get("stage0_enable", True)):
        ...
        print("[Stage0-ONCE] skipped (stage0_enable=False). Encoder starts from random init.")
    ...
    for r in range(CFG["repeats"]):
        ...
        enc2, clf2, log = train_stage2_tnbc_Aplus(
            ...
            stage2_unfreeze_lastlayer=bool(CFG.get("stage2_unfreeze_lastlayer", True)),
            stage2_phase1_encoder_eval=bool(CFG.get("stage2_phase1_encoder_eval", True)),
            device=device,
        )
...
if __name__ == "__main__":
    ...
    CFG = dict(
        ...
        # ---- run mode ----
        run_ablation_suite=False,  # set True to run the ablation batch (see ABLATIONS below)
        ...
        # ---- reuse (recommend False when changing topK or ablations) ----
        reuse_stage0=False,
        reuse_stage1=False,
        ...
        # ---- paired ablation reporting (only used when run_ablation_suite=True) ----
        paired_metric="outer_auc",
        paired_bootstrap_n=20000,
        ...
        # ---- Stage0 (DAE) ----
        stage0_enable=True,        # ablation knob: False -> skip DAE pretrain (random init encoder)
        ae_hidden=1024,
        ae_lr=1e-3,
        ae_epochs=50,
        ae_batch=128,
        ...
        stage2_lr_enc_mult=0.02,   # lastlayer lr = head_lr * mult
        lambda_l2sp=0.001,

        # ablation knobs:
        stage2_unfreeze_lastlayer=True,      # False -> keep encoder frozen for all epochs (head-only)
        stage2_phase1_encoder_eval=True,     # False -> keep dropout noise in phase1 (encoder frozen but enc.train())
    ...
    # -------------------------
    # Paired ablation utilities (paired across repeats/seeds)
    # -------------------------
    def _bootstrap_ci_mean(x, n=20000, seed=0):
        x = np.asarray(x, dtype=float)
        x = x[~np.isnan(x)]
        if x.size == 0:
            return float("nan"), float("nan"), float("nan")
        rng = np.random.default_rng(int(seed))
        boots = rng.choice(x, size=(int(n), x.size), replace=True).mean(axis=1)
        lo, hi = np.quantile(boots, [0.025, 0.975])
        return float(x.mean()), float(lo), float(hi)

    def _paired_delta_summary(full_out_root: Path, abla_out_root: Path, metric: str = "outer_auc",
                              bootstrap_n: int = 20000, bootstrap_seed: int = 0):
        '''
        Returns:
          delta_df: merged per-repeat table with columns [rep, seed, metric_full, metric_abla, delta]
          stats: dict with mean/std/min/max and 95% bootstrap CI of mean
        Pairing key is (rep, seed), which is stable because seed is deterministically set as CFG['seed'] + rep*100.
        '''
        full = pd.read_csv(full_out_root / "summary.csv")
        abla = pd.read_csv(abla_out_root / "summary.csv")

        for col in ["rep", "seed", metric]:
            if col not in full.columns:
                raise ValueError(f"[PAIRED] full summary.csv missing column: {col} @ {full_out_root}")
            if col not in abla.columns:
                raise ValueError(f"[PAIRED] abla summary.csv missing column: {col} @ {abla_out_root}")

        m = full[["rep", "seed", metric]].merge(
            abla[["rep", "seed", metric]],
            on=["rep", "seed"],
            suffixes=("_full", "_abla"),
            how="inner",
        )
        if m.empty:
            raise ValueError(f"[PAIRED] No matched (rep,seed) pairs between {full_out_root} and {abla_out_root}")

        m["delta"] = m[f"{metric}_full"].astype(float) - m[f"{metric}_abla"].astype(float)

        d = m["delta"].to_numpy(dtype=float)
        mean = float(np.nanmean(d))
        std = float(np.nanstd(d))
        dmin = float(np.nanmin(d))
        dmax = float(np.nanmax(d))
        _, lo, hi = _bootstrap_ci_mean(d, n=bootstrap_n, seed=bootstrap_seed)
    ...
    def _save_delta_hist(delta_values, out_pdf: Path, xlabel: str):
        try:
            plt.figure()
            plt.hist(delta_values, bins=15)
            plt.axvline(float(np.nanmean(delta_values)), linestyle="--")
            plt.xlabel(xlabel)
            plt.ylabel("count")
            plt.tight_layout()
            plt.savefig(out_pdf)
            ...
            print(f"[PAIRED] warn: failed to save hist {out_pdf}: {e}")

    def run_ablation_suite(base_cfg: Dict):
        suite_root = Path(base_cfg["out_dir"]).with_name(Path(base_cfg["out_dir"]).name + "_ABLA")
        ensure_dir(suite_root)

        # Keep this list short and "load-bearing" for the paper.
        # Each ablation will run full repeats and generate its own summary.csv + crossfit_oos_pred.csv.
        ABLATIONS = [
            ("full", {}),
            ("w_o_grl", {"stage1_use_grl": False}),
            ("w_o_stage0", {"stage0_enable": False, "reuse_stage0": False, "reuse_stage1": False}),
            ("w_o_Aplus_lastlayer", {"stage2_unfreeze_lastlayer": False}),
            ("w_o_l2sp", {"lambda_l2sp": 0.0}),
            ("mixup_on", {"use_mixup": True}),
            ("w_o_phase1_enc_eval", {"stage2_phase1_encoder_eval": False}),
        ]
        ...
        for name, override in ABLATIONS:
            ...
            # Put each ablation into its own folder to avoid any accidental overwrite.
            cfg["out_dir"] = str(suite_root / name)

            # Strongly recommend no reuse in ablation sweeps.
            cfg["reuse_stage0"] = False
            cfg["reuse_stage1"] = False

            print("\n" + "#" * 100)
            print(f"[ABLA] {name} -> out_dir={cfg['out_dir']}")
            print("#" * 100 + "\n")
            ...
            summ_path = out_root / "summary.csv"
            cf_path = out_root / "crossfit_oos_pred.csv"

            summ = pd.read_csv(summ_path)
            aucs = summ["outer_auc"].astype(float).values

            cf_auc = float("nan")
            if cf_path.exists():
                cf = pd.read_csv(cf_path)
                ok = cf["p_oos_mean"].notna().values
                if ok.sum() >= 10 and len(np.unique(cf.loc[ok, "y"].values)) == 2:
                    cf_auc = roc_auc_score(cf.loc[ok, "y"].values, cf.loc[ok, "p_oos_mean"].values)

            rows.append({
                "ablation": name,
                "outer_auc_mean": float(np.nanmean(aucs)),
                "outer_auc_std": float(np.nanstd(aucs)),
                "outer_auc_min": float(np.nanmin(aucs)),
                "outer_auc_max": float(np.nanmax(aucs)),
                "crossfit_auc": cf_auc,
...
[ABLA] saved: {out_csv}")
        ...
        # -------------------------
        # Paired ablation stats vs "full" (per-repeat paired Δmetric)
        # Outputs:
        #   - paired_ablation_summary.csv  (Δ mean ± 95% bootstrap CI + std, n_pairs)
        #   - paired_delta__<ablation>.csv (per-repeat paired deltas)
        #   - paired_delta__<ablation>.pdf (delta histogram)
        # -------------------------
        try:
            df_rows = pd.DataFrame(rows)
            if "full" in set(df_rows["ablation"].tolist()):
                full_root = Path(df_rows.loc[df_rows["ablation"] == "full", "out_root"].iloc[0])
                metric = str(base_cfg.get("paired_metric", "outer_auc"))
                boot_n = int(base_cfg.get("paired_bootstrap_n", 20000))
                boot_seed = int(base_cfg.get("paired_bootstrap_seed", 0))

                paired_rows = []
                for _, rr in df_rows.iterrows():
                    name = str(rr["ablation"])
                    if name == "full":
                        continue
                    ab_root = Path(rr["out_root"])
                    delta_df, st = _paired_delta_summary(
                        full_out_root=full_root,
                        abla_out_root=ab_root,
                        metric=metric,
                        bootstrap_n=boot_n,
                        bootstrap_seed=boot_seed,
                    )

                    delta_csv = suite_root / f"paired_delta__{name}.csv"
                    delta_df.to_csv(delta_csv, index=False)

                    delta_pdf = suite_root / f"paired_delta__{name}.pdf"
                    _save_delta_hist(
                        delta_values=delta_df["delta"].to_numpy(dtype=float),
                        out_pdf=delta_pdf,
                        xlabel=f"Δ{metric} (full - {name})",
                    )
                    ...
                    paired_rows.append({
                        ...
                        "delta_min": st["delta_min"],
                        "delta_max": st["delta_max"],
                        "full_out_root": str(full_root),
                        "abla_out_root": str(ab_root),
                        "paired_delta_csv": str(delta_csv),
                        "paired_delta_pdf": str(delta_pdf),
                    })

                paired_out = suite_root / "paired_ablation_summary.csv"
                pd.DataFrame(paired_rows).to_csv(paired_out, index=False)
                print(f"[PAIRED] metric={metric} bootstrap_n={boot_n} saved: {paired_out}")
            else:
                print("[PAIRED] skip: no 'full' in ABLATIONS list.")
        except Exception as e:
            print(f"[PAIRED] warn: failed to compute paired deltas: {e}")

    if bool(CFG.get("run_ablation_suite", False)):
