...
def train_stage2_tnbc_Aplus(
    ...
    l2sp_den = 1  # number of trainable encoder params used in L2SP (set at phase2)
    ...
    for ep in range(epochs):
        ...
        if bool(stage2_unfreeze_lastlayer) and ep == phase1_epochs:
            ...
            l2sp_den = sum(v.numel() for v in ref_last.values()) or 1
        ...
        for xb, yb, _ in dl_lab_tr:
            ...
            if ep < phase1_epochs:
                ...
                # L2-SP on last layer (use SUM of squared differences; tune lambda accordingly)
                loss_l2sp = torch.tensor(0.0, device=device)
                l2sp_numel = 0
                for name, p in enc.named_parameters():
                    if p.requires_grad:
                        d = p - ref_last[name]
                        loss_l2sp = loss_l2sp + (d * d).sum()
                        l2sp_numel += d.numel()
        ...
        l2sp_sum = float(np.mean(l2sps)) if len(l2sps) else 0.0
        l2sp_mean = l2sp_sum / float(l2sp_den) if l2sp_den else 0.0
        l2sp_scaled = float(lambda_l2sp * l2sp_sum) if ep >= phase1_epochs else 0.0
        print(
            f"[Stage2] ep={ep:03d} TNBC-val AUC={auc_tnbc:.4f} | OTH-val AUC={auc_oth:.4f} "
            f"| sup={np.mean(losses):.4f} l2sp_sum={l2sp_sum:.3e} l2sp_mean={l2sp_mean:.3e} scaled={l2sp_scaled:.3e} "
...
def main(CFG: Dict):
    ...
    ensure_dir(out_root)
    # Dump resolved config for reproducibility / debugging
    try:
        (out_root / "config.json").write_text(json.dumps(CFG, indent=2, sort_keys=True), encoding="utf-8")
    except Exception as e:
        print(f"[WARN] failed to dump config.json: {e}")
...
if __name__ == "__main__":
    ...
    CFG = dict(
        ...
        lambda_l2sp=1e-6,  # NOTE: L2SP uses SUM of squared diffs; start with 1e-6~1e-5 and tune
