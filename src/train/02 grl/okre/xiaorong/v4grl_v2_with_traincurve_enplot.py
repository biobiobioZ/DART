...
def plot_stage1_adversarial_curves(log_csv: str, out_png: str, stable_loss: float = 0.693):
    """
    Generate a Fig.3-like training-process plot (two panels):
      (a) loss curves
      (b) domain loss + mean domain probability with theoretical stable lines
    """
    ...
    dom_prob = df["dom_prob_mean"].to_numpy()
    val_task_loss = df.get("val_task_loss", pd.Series([np.nan] * len(df))).to_numpy()

    fig, axes = plt.subplots(2, 1, figsize=(8.0, 7.0), sharex=True)

    # (a) loss curves
    ax = axes[0]
    ax.plot(it, dom_loss, label="Domain classifier loss")
    ax.plot(it, total_loss, label="Predictor (train total loss)")
    ax.plot(it, task_loss, label="Predictor (train task loss, source)")

    m = ~np.isnan(val_task_loss)
    if np.any(m):
        ax.plot(it[m], val_task_loss[m], label="Predictor (val task loss)")

    ax.axhline(stable_loss, linestyle="--", linewidth=1.0)
    ax.text(it.max() * 0.72, stable_loss + 0.01, f"Theoretical stable loss{stable_loss:.3f}")

    ax.set_ylabel("Loss")
    ax.grid(True, linewidth=0.3, alpha=0.4)
    ax.legend(loc="upper right", frameon=False)

    # (b) domain loss + probability scatter
    ax = axes[1]
    ax.scatter(it, dom_loss, s=14, label="Domain classifier loss")
    ax.scatter(it, dom_prob, s=14, marker="s", label="Domain prob (mean)")

    ax.axhline(stable_loss, linestyle="--", linewidth=1.0)
    ax.axhline(0.5, linestyle="--", linewidth=1.0)
    ax.text(it.max() * 0.62, stable_loss + 0.01, f"Theoretical stable loss{stable_loss:.3f}")
    ax.text(it.max() * 0.62, 0.5 + 0.01, "Theoretical prob 0.5")
...
def main(CFG: Dict):
    ...
    try:
        ...
        if log_csv.exists():
            out_png = out_root / "fig_stage1_grl_training.png"
