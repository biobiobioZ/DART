...
def plot_stage1_adversarial_curves(log_csv: str,
                               ...
                               ma_window: int = 10):
"""
Paper-friendly DANN/GRL training dynamics plot (clean version).

Panel (a): task losses (train/val) with smoothing.
Panel (b): domain loss (left y) + domain accuracy and lambda schedule (right y), all smoothed.
"""
...
# val is sparse (NaNs). Smooth but plot only where available.
val_mask = np.isfinite(val_task_loss)
val_task_loss_s = _smooth_ma(val_task_loss, ma_window)

dom_acc_mask = np.isfinite(dom_acc)
dom_acc_s = _smooth_ma(dom_acc, ma_window) if dom_acc_mask.any() else dom_acc

lambd_mask = np.isfinite(lambd)
lambd_s = _smooth_ma(lambd, ma_window) if lambd_mask.any() else lambd

fig, axes = plt.subplots(2, 1, figsize=(7.2, 5.8), sharex=True)

# ---------------- (a) Task losses ----------------
ax = axes[0]
ax.plot(it, task_loss_s, linewidth=2.0, label="Task loss (train, source)")
if val_mask.any():
    ax.plot(it[val_mask], val_task_loss_s[val_mask], linewidth=2.0, label="Task loss (val)")
ax.set_ylabel("Loss")
...
# ---------------- (b) Domain dynamics ----------------
ax = axes[1]
ax.plot(it, dom_loss_s, linewidth=2.0, label="Domain loss")
ax.axhline(stable_loss, linestyle="--", linewidth=1.0, label=f"Theoretical loss ({stable_loss:.3f})")
ax.set_ylabel("Loss")
ax.grid(True, linewidth=0.3, alpha=0.30)

ax2 = ax.twinx()
if dom_acc_mask.any():
    ax2.plot(it, dom_acc_s, linewidth=2.0, label="Domain acc")
    ax2.axhline(0.5, linestyle="--", linewidth=1.0, label="Chance acc (0.5)")
    ax2.set_ylabel("Accuracy")

if lambd_mask.any():
    # Keep lambda visually subtle to avoid clutter
    ax2.plot(it, lambd_s, linestyle=":", linewidth=2.0, alpha=0.8, label="Lambda (schedule)")

axes[1].set_xlabel("Iteration")

# Unified legend for panel (b), placed where it won't overlap too much.
...
def main(CFG: Dict):
    ...
    try:
        ...
        if log_csv.exists():
            out_png = out_root / "fig_stage1_grl_training_paper_v2.png"
