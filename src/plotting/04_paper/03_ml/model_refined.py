...
- Save (under OUTDIR):
  ...
  - metrics heatmaps (paper-friendly)
  - confusion matrices (OTH-test and TNBC-test) for the best TNBC model
...
OUTDIR = "out_ml_baselines_clickrun2"
...
def _plot_metrics_heatmap(df_metrics: pd.DataFrame, title: str, out_png: Path, out_pdf: Path):
    ...
    im = ax.imshow(
        ...
        aspect="auto",
        cmap="Blues",  # 蓝色系
        vmin=0.0, vmax=1.0  # 指标基本都在[0,1]，锁定范围更美观一致
    )
...
def _plot_confusion_matrix(
    ...
    """Plot a 2x2 confusion matrix with count + row-normalized percentage."""
    cm = np.asarray(cm)
    if cm.shape != (2, 2):
        warnings.warn(f"[WARN] confusion matrix shape is {cm.shape}, expected (2,2). Skip plot: {title}")
        return
    ...
    ax = fig.add_subplot(111)
    im = ax.imshow(cm_pct, vmin=0.0, vmax=1.0)
...
def main():
    ...
    # Confusion matrices for the best TNBC model (paper-friendly)
    best = summary_df.iloc[0]["model"]
    best_pipe = pipes.get(best)
    if best_pipe is not None:
        cm_oth = confusion_matrix(y_test, best_pipe.predict(X_test))
        cm_tnbc = confusion_matrix(y_tnbc_enc, best_pipe.predict(X_tnbc))

        _plot_confusion_matrix(
            cm_oth,
            title=f"Confusion Matrix on OTH Internal Test | {best}",
            out_png=outdir / f"cm_oth_test_{best}.png",
            out_pdf=outdir / f"cm_oth_test_{best}.pdf",
            class_names=("0", "1"),
        )
        _plot_confusion_matrix(
            cm_tnbc,
            title=f"Confusion Matrix on TNBC External Test | {best}",
            out_png=outdir / f"cm_tnbc_test_{best}.png",
            out_pdf=outdir / f"cm_tnbc_test_{best}.pdf",
            class_names=("0", "1"),
        )
        print(f"[OK] saved confusion matrices -> cm_oth_test_{best}.(png/pdf), cm_tnbc_test_{best}.(png/pdf)")
