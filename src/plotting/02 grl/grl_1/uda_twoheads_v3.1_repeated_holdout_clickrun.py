...
#
# ✅ Added Stage3 (Target-only, source-free SSDA with EMA teacher):
# - Start from Stage2 best_target
# - Use ONLY TNBC microarray labeled train anchors + TNBC microarray unlabeled
# - EMA teacher generates pseudo labels (balanced + confident)
# - Student learns with strong augmentation consistency
# - Select best checkpoint by TNBC_val AUC, then test on TNBC_test
...
# =========================
# ✅ Stage3 helpers (EMA + strong aug)
# =========================
@torch.no_grad()
def ema_update_(teacher: nn.Module, student: nn.Module, decay: float):
    tsd = teacher.state_dict()
    ssd = student.state_dict()
    for k in tsd.keys():
        tsd[k].mul_(decay).add_(ssd[k], alpha=(1.0 - decay))
    teacher.load_state_dict(tsd)

def strong_aug_expr(xb: torch.Tensor, mask_ratio: float, noise_std: float):
...
def stage3_target_only_ema_ssda(
    model: nn.Module,
    focal: nn.Module,
    center: nn.Module,
    tnbcArrLabTrain: DataLoader,  # anchors (train)
    tnbcArrUL: DataLoader,        # unlabeled (microarray UL)
    tnbcArrLabVal: DataLoader,    # selection (val)
    device: torch.device,
    cfg: Dict[str, Any],
    run_out: Path,
    repeat_id: int,
):
    if not cfg.get("stage3_enable", True):
        print(f"[REP {repeat_id:02d}] [Stage3] disabled (stage3_enable=False).")
        return None

    stage3_epochs = int(cfg.get("stage3_epochs", 0))
    if stage3_epochs <= 0:
        print(f"[REP {repeat_id:02d}] [Stage3] skipped (stage3_epochs<=0).")
        return None

    # freeze domain heads (not used in Stage3)
    for p in model.platform.parameters():
        p.requires_grad = False
    for p in model.cancer.parameters():
        p.requires_grad = False

    lr3 = float(cfg.get("stage3_lr", cfg["lr"] * 0.5))
    wd3 = float(cfg.get("stage3_weight_decay", cfg["weight_decay"]))

    opt = torch.optim.Adam(
        [
            {"params": [p for p in model.parameters() if p.requires_grad]},
            {"params": center.parameters(), "lr": lr3 * 0.5},
        ],
        lr=lr3,
        weight_decay=wd3,
    )

    ema_decay = float(cfg.get("stage3_ema_decay", 0.999))
    warmup = float(cfg.get("stage3_warmup", 0.10))

    lam_tlab = float(cfg.get("stage3_lambda_tlab", 1.0))
    lam_pl = float(cfg.get("stage3_lambda_pl", 0.6))
    lam_ent = float(cfg.get("stage3_lambda_entropy", 0.0))
    lam_ort = float(cfg.get("stage3_lambda_ortho", cfg.get("lambda_ortho", 0.0)))

    tau_start = float(cfg.get("stage3_tau_start", 0.90))
    tau_end   = float(cfg.get("stage3_tau_end", 0.80))

    mask_ratio = float(cfg.get("stage3_mask_ratio", 0.05))
    noise_std  = float(cfg.get("stage3_noise_std", 0.02))

    s3_steps = int(cfg.get("stage3_steps_per_epoch", 0))
    if s3_steps <= 0:
        s3_steps = max(1, min(len(tnbcArrUL), max(1, len(tnbcArrLabTrain))))

    total_steps = max(1, s3_steps * stage3_epochs)
    gstep = 0
    ...
    # avoid drop_last=True discarding anchors in Stage3
    anchor_ds = tnbcArrLabTrain.dataset
    anchor_loader = DataLoader(anchor_ds, batch_size=cfg["batch"], shuffle=True, drop_last=False)
    it_a = cycle_loader(anchor_loader)
    it_u = cycle_loader(tnbcArrUL)

    best_auc = -1.0
    best_epoch = -1
    ckpt_s3 = run_out / "checkpoint_stage3_best_target.pt"
    ...
    for ep in range(stage3_epochs):
        ...
        for _ in range(s3_steps):
            ...
            r_all = ramp(prog, warm=warmup)
            tau = tau_start + (tau_end - tau_start) * prog
            ep_tau = float(tau)

            xb_t, yb_t, *_ = next(it_a)
            xb_t, yb_t = xb_t.to(device), yb_t.to(device)

            xb_u, *_ = next(it_u)
            xb_u = xb_u.to(device)

            # supervised on anchors
            logits_t, _, _, z_all_t, _, _, z_inv_t, z_bc_t = model(xb_t, None, None)
            L_sup = focal(logits_t, yb_t) + cfg["lambda_center"] * center(z_all_t, yb_t)
            L_sup = L_sup * lam_tlab

            # teacher pseudo labels on WEAK view
            with torch.no_grad():
                teacher.eval()
                t_logits_u, _, _, _, _, _, _, _ = teacher(xb_u, None, None)
                pt = torch.softmax(t_logits_u, 1)

            mask_pl, pseudo, conf, mask_rate, pos_rate = pick_balanced_pseudolabels(
                pt, tau=tau,
                max_per_class=int(cfg.get("stage3_pl_max_per_class", cfg.get("pl_max_per_class", 12))),
                min_per_class=int(cfg.get("stage3_pl_min_per_class", cfg.get("pl_min_per_class", 2)))
            )
            ep_mask_rate.append(float(mask_rate))
            ep_pos_rate.append(float(pos_rate))

            # student on STRONG view
            xb_u_s = strong_aug_expr(xb_u, mask_ratio=mask_ratio, noise_std=noise_std)
            logits_u, _, _, _, _, _, z_inv_u, z_bc_u = model(xb_u_s, None, None)

            if lam_pl > 0 and mask_pl.any():
                L_pl = F.cross_entropy(logits_u[mask_pl], pseudo[mask_pl]) * (lam_pl * r_all)
            else:
                L_pl = 0.0

            if lam_ent > 0:
                pu = torch.softmax(logits_u, 1)
                if cfg.get("stage3_ent_on_conf", True) and mask_pl.any():
                    pu_sel = pu[mask_pl]
                else:
                    pu_sel = pu
                L_ent = (-(pu_sel * (pu_sel.clamp_min(1e-8)).log()).sum(dim=1)).mean()
                L_ent = L_ent * (lam_ent * r_all)
            else:
                L_ent = 0.0

            if lam_ort > 0:
                L_ortho = lam_ort * (orth_loss(z_inv_t, z_bc_t) + orth_loss(z_inv_u, z_bc_u)) * 0.5
            else:
                L_ortho = 0.0

            loss = L_sup + L_pl + L_ent + L_ortho
            ...
            ema_update_(teacher, model, ema_decay)
            gstep += 1

        auc_val, _, _ = evaluate_auc(model, tnbcArrLabVal, device)
        auc_log.append(float(auc_val))
        mr = float(np.mean(ep_mask_rate)) if ep_mask_rate else 0.0
        pr = float(np.mean(ep_pos_rate)) if ep_pos_rate else 0.0
        mask_log.append(mr)
        pos_log.append(pr)
        tau_log.append(ep_tau if ep_tau is not None else float("nan"))

        print(f"[REP {repeat_id:02d}] [Stage3] epoch {ep} TNBC-val AUC={auc_val:.4f} "
              f"(tau~{ep_tau:.2f}) warm_r={ramp((gstep-1)/max(1,total_steps-1), warmup):.2f} mask={mr:.3f} mask&1={pr:.3f}")

        if (not np.isnan(auc_val)) and float(auc_val) > best_auc:
            best_auc = float(auc_val)
            best_epoch = ep + 1
            torch.save(model.state_dict(), ckpt_s3)

    if auc_log:
        _save_curve_csv(auc_log, run_out / "stage3_auc_tnbc_val.csv", "AUC")
        _save_line_plot(auc_log, "AUC", f"Stage3 TNBC-val AUC (rep {repeat_id})", run_out / "stage3_auc_tnbc_val.png")
    pd.DataFrame({
        "epoch": np.arange(1, len(mask_log) + 1),
        "mask_rate": mask_log,
        "pos_rate": pos_log,
        "tau": tau_log
    }).to_csv(run_out / "stage3_pl_stats.csv", index=False)

    if ckpt_s3.exists():
        model.load_state_dict(safe_torch_load(ckpt_s3, device), strict=True)

    return {"stage3_best_val_auc": float(best_auc), "stage3_best_epoch": int(best_epoch)}
...
def run_one_repeat(cfg: Dict[str, Any], repeat_id: int, out_dir: Path,
    ...
    - Stage3 target-only EMA SSDA (optional)
    ...
    # =========================
    # ✅ Stage3: Target-only SSDA with EMA teacher
    # =========================
    ...
    stage3_info = stage3_target_only_ema_ssda(
        model=model,
        focal=focal,
        center=center,
        tnbcArrLabTrain=tnbcArrLabTrain,
        tnbcArrUL=tnbcArrUL,
        tnbcArrLabVal=tnbcArrLabVal,
        device=device,
        cfg=cfg,
        run_out=run_out,
        repeat_id=repeat_id,
    ...
    # =========================
    # Final eval on TNBC test (prefer Stage3 best_target)
    # =========================
    ckpt_s3 = run_out / "checkpoint_stage3_best_target.pt"
    ckpt_othval = run_out / "checkpoint_uda_best_othval.pt"

    if ckpt_s3.exists():
        chosen = "stage3_best_target"
        model.load_state_dict(safe_torch_load(ckpt_s3, device), strict=True)
    elif ckpt_target.exists():
        chosen = "uda_best_target"
        model.load_state_dict(safe_torch_load(ckpt_target, device), strict=True)
    elif ckpt_othval.exists():
        chosen = "uda_best_othval"
    ...
    metrics = {
        ...
        "stage3_best_epoch": int(stage3_info["stage3_best_epoch"]) if stage3_info else -1,
        "stage3_best_val_auc": float(stage3_info["stage3_best_val_auc"]) if stage3_info else float("nan"),
        "OTH_val": m_oth,
        "TNBC_val": m_val,
        "TNBC_test": m_test,
        ...
        "cfg_snapshot": {
            ...
            "stage3_enable": bool(cfg.get("stage3_enable", True)),
            "stage3_epochs": int(cfg.get("stage3_epochs", 0)),
            "stage3_tau_start": float(cfg.get("stage3_tau_start", float("nan"))),
            "stage3_tau_end": float(cfg.get("stage3_tau_end", float("nan"))),
        }
    }
...
def main():
    ...
    df = pd.DataFrame([{
        ...
        "stage3_best_epoch": m.get("stage3_best_epoch", -1),
        "stage3_best_val_auc": m.get("stage3_best_val_auc", float("nan")),
...
CFG = dict(
    ...
    repeats=10,
    tnbc_test_ratio=0.20,
    tnbc_val_ratio_within_trainval=0.20,

    # strictness & speed
    strict_no_peek=True,
    reuse_pretrain_encoder=False,
    use_tnbc_train_in_ae_pool=True,
    ...
    # =========================
    # ✅ Stage3: Target-only EMA SSDA (new)
    # =========================
    stage3_enable=True,
    stage3_epochs=40,
    stage3_steps_per_epoch=0,      # 0=auto
    stage3_lr=8e-5,
    stage3_weight_decay=1e-4,
    stage3_ema_decay=0.999,

    stage3_warmup=0.10,
    stage3_lambda_tlab=1.0,
    stage3_lambda_pl=0.6,
    stage3_lambda_entropy=0.00,
    stage3_ent_on_conf=True,
    stage3_lambda_ortho=0.02,

    stage3_tau_start=0.90,
    stage3_tau_end=0.80,
    stage3_pl_max_per_class=12,
    stage3_pl_min_per_class=2,

    stage3_mask_ratio=0.05,
    stage3_noise_std=0.02,
)
